...
public class ECSubscribesToDSBBootstrapServiceImpl implements BootstrapService {
     ...
     */
    public List<Subscription> bootstrap(String providerEndpoint, String subscriberEndpoint)
        ...
        for (Topic topic : topics) {
            ...
            Subscription bean = createResources(topic, providerEndpoint, subscriberEndpoint);
            if (bean != null) {
                result.add(bean);
            }
        }
    ...
    private Subscription createResources(Topic topic, String providerEndpoint, String subscriberEndpoint) {
        ...
        EventCloudsManagementWsApi client = eventCloudClientFactory.getClient(subscriberEndpoint);

        QName topicName = new QName(topic.getNs(), topic.getName(), topic.getPrefix());

        String stream = StreamHelper.getStreamName(topicName);
        List<String> publishURLs = client.getPublishWsnServiceEndpointUrls(stream);
        ...
        if (needsToSubscribe(stream)) {
            ...
            try {
                ...
                result = topicManager.subscribe(providerEndpoint, topicName, subscriber);
