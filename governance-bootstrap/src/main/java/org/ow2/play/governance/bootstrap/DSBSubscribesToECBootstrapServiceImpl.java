...
public class DSBSubscribesToECBootstrapServiceImpl implements BootstrapService {
    ...
    @Override
    public List<Subscription> bootstrap(String eventCloudEndpoint, String subscriberEndpoint)
        ...
        for (Topic topic : topics) {
            try {
                Subscription subscription = createResources(topic, eventCloudEndpoint, subscriberEndpoint);
    ...
    protected Subscription createResources(Topic topic, String eventCloudEndpoint, String subscriberEndpoint)
            throws BootstrapFault {
        ...
        EventCloudsManagementWsApi client = eventCloudClientFactory.getClient(eventCloudEndpoint);
        ...
        if (created) {
            ...
            log.log("EventCloud has been already created for stream %s", streamName);
        }
        ...
        if (needsToSubscribe(streamName)) {
            ...
            if (alreadySubscribed(topic, eventCloudEndpoint, subscriberEndpoint)) {
                ...
                if (endpoints == null || endpoints.size() == 0) {
                    log.log("Can not find any subscribe endpoint in the EC for stream %s", streamName);
                } else {
                    log.log("Let's subscribe to eventcloud for stream %s", streamName);

                    result = topicManager.subscribe(endpoints.get(0), topicName, subscriberEndpoint);
                    log.log("DSB subscribed to EC : " + result);
                }
            }
        } else {
            log.log("Do not need to subscribe to eventcloud for stream %s", streamName);
        }
