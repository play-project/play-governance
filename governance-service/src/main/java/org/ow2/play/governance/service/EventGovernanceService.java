...
public class EventGovernanceService implements EventGovernance {
	...
	public String createTopic(Topic topic) throws GovernanceExeption {
		...
		// each one implementing the createTopic(Topic) operation so we can add
		// more at runtime for example.
		try {
	...
	public String createPublisherTopic(Topic topic) throws GovernanceExeption {
		...
		String subscriber = (publishURLs != null && publishURLs.size() > 0) ? publishURLs.get(0)
				: null;
		...
		subscription
				.setProvider(getEndpoint(org.ow2.play.service.registry.api.Constants.EC_TO_DSB_DSB));
	...
	public String createSubscriberTopic(final Topic topic) throws GovernanceExeption {
		...
		if (mr == null) {
			...
			// TODO : Fix constants
			mr.getMetadata().add(
					new Metadata(org.ow2.play.metadata.api.Constants.COMPLEX_EVENT, new Data(
							Type.LITERAL,
							Boolean.toString(true))));
			mr.getMetadata().add(
					new Metadata(org.ow2.play.metadata.api.Constants.DSB_NEEDS_TO_SUBSCRIBE,
							new Data(
	...
	public List<Topic> getTopics() throws GovernanceExeption {
		...
		if (resources != null) {
			for (MetaResource r : resources) {
				...
				List<Metadata> md = new ArrayList<Metadata>(Collections2.filter(r.getMetadata(),
						new Predicate<Metadata>() {
							@Override
							public boolean apply(Metadata meta) {
								return meta.getName() != null
										&& meta.getName().equals(Constants.QNAME_PREFIX_URL);
							}
						}));
	 ...
	 * 
	 * @see
	 * org.ow2.play.governance.api.EventGovernance#getTopicsFromName(java.lang
	 * .String)
	 */
	@Override
